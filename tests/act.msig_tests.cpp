...
public:
   eosio_msig_tester() {
      create_accounts( { N(act.msig), N(act.stake), N(act.ram), N(act.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(act.msig), contracts::msig_wasm() );
      set_abi( N(act.msig), contracts::msig_abi().data() );
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(act), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(act), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(act.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(act.msig), name, accounts, data );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   // required to set up the link between (act active) and (act.prods active)
   //
   //                  act active
   //                       |
   //             act.prods active (2/3 threshold)
   //             /         |        \             <--- implicitly updated in onblock action
   // alice active     bob active   carol active

   set_authority(N(act), "active", authority(1,
      vector<key_weight>{{get_private_key("act", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(act.prods), config::active_name}, 1}}), "owner",
      { { N(act), "active" } }, { get_private_key( N(act), "active" ) });

   set_producers( {N(alice),N(bob),N(carol)} );
   produce_blocks(50);

   create_accounts( { N(act.token), N(act.rex) } );
   set_code( N(act.token), contracts::token_wasm() );
   set_abi( N(act.token), contracts::token_abi().data() );

   create_currency( N(act.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance("act") + get_balance("act.ramfee") + get_balance("act.stake") + get_balance("act.ram") );
   ...
   produce_blocks();
   create_account_with_resources( N(alice1111111), N(act), core_sym::from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(act), core_sym::from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(act), core_sym::from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(act), config::active_name}};
   ...
   // execute by alice to replace the act system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect(
   [&]( std::tuple<const transaction_trace_ptr&, const signed_transaction&> p ) {
      const auto& t = std::get<0>(p);
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(act), core_sym::from_string("1.0000"), false ),
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, eosio_msig_tester ) try {
   ...
   // set up the link between (act active) and (act.prods active)
   set_authority(N(act), "active", authority(1,
      vector<key_weight>{{get_private_key("act", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(act.prods), config::active_name}, 1}}), "owner",
      { { N(act), "active" } }, { get_private_key( N(act), "active" ) });
   ...
   create_currency( N(act.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   issue(config::system_account_name, core_sym::from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance( "act" ) );
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, eosio_msig_tester ) try {
   set_code( N(act.msig), contracts::util::msig_wasm_old() );
   set_abi( N(act.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();

   //propose with old version of act.msig
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   push_action( N(alice), N(propose), mvo()
                  ("proposer",      "alice")
                  ("proposal_name", "first")
